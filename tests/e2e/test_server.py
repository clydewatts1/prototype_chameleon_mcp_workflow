...
from chameleon_workflow_engine.server import app, db_manager as server_db_manager
...
def test_instantiate_workflow_api(test_client):
    ...
    with test_client.db_manager.get_instance_session() as session:
        uows = session.query(UnitsOfWork).filter(
            UnitsOfWork.instance_id == workflow_uuid
        ).all()
...
def test_checkout_work_api_success(test_client):
    ...
    initial_context = {
        "task_id": "TASK-002",
        "task_data": "Test data for checkout"
    }
    
    instantiate_response = test_client.post("/workflow/instantiate", json={
        "template_id": str(test_client.template_id),
        "initial_context": initial_context
    })
    ...
    with test_client.db_manager.get_instance_session() as session:
        ...
        local_workflow = session.query(Local_Workflows).filter(
            Local_Workflows.instance_id == uuid.UUID(workflow_id)
        ).first()
    ...
    checkout_response = test_client.post("/workflow/checkout", json={
        "actor_id": actor_id,
    ...
    with test_client.db_manager.get_instance_session() as session:
        uow = session.query(UnitsOfWork).filter(
            UnitsOfWork.uow_id == uuid.UUID(uow_id)
        ).first()
        ...
        assert uow.last_heartbeat is not None, \
            "last_heartbeat should be set after checkout"
...
def test_checkout_work_api_empty(test_client):
    ...
    with test_client.db_manager.get_template_session() as session:
        ...
        beta_role = session.query(Template_Roles).filter(
            Template_Roles.role_type == RoleType.BETA.value
        ).first()
    ...
    first_checkout = test_client.post("/workflow/checkout", json={
        "actor_id": actor_1,
        "role_id": beta_role_id
    })
    ...
    second_checkout = test_client.post("/workflow/checkout", json={
        "actor_id": actor_2,
        "role_id": beta_role_id
    })
    
    # Assert 204 No Content
    assert second_checkout.status_code == 204, \
        f"Expected 204 No Content, got {second_checkout.status_code}"
    
    # Verify no response body (or minimal body)
    assert second_checkout.text == "" or second_checkout.text == "null", \
        "Expected empty response body for 204 No Content"
    
    print(f"✓ test_checkout_work_api_empty passed - received 204 No Content")
...
def test_submit_work_api(test_client):
    ...
    submit_response = test_client.post("/workflow/submit", json={
        "uow_id": uow_id,
        "actor_id": actor_id,
    ...
    with test_client.db_manager.get_instance_session() as session:
        ...
        all_attrs = session.query(UOW_Attributes).filter(
            UOW_Attributes.uow_id == uuid.UUID(uow_id)
        ...
        assert len(attr_versions['status']) >= 2, \
...
def test_failure_reporting_api(test_client):
    ...
    failure_response = test_client.post("/workflow/failure", json={
        "uow_id": uow_id,
        "actor_id": actor_id,
